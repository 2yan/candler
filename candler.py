...
class Candler():
    ...
    def __init__(self,product_id, candle_width_seconds = 60.0000000001 , url = 'https://api.gdax.com'):
        ...
        self.time_difference = pd.to_datetime(self.get('/time')['iso']) - datetime.now()
    ...
    def get_candles(self, start=None, end=None, granularity=None):
        ...
        candles = self.get('/products/{}/candles'.format(str(self.product_id)), params=params)
...
for iden in ids:  
    processes[iden] = multiprocessing.Process(target = doit, args =(iden,))
    processes[iden].start()